...
export class S3Service {
  ...
  /**
   * Sube imagen para WhatsApp Estado
   * Agregar este método al final de S3Service (s3.service.ts)
   */
  async uploadWhatsAppImage(imageBuffer: Buffer): Promise<string> {
    try {
      const fecha = new Date();
      const year = fecha.getFullYear();
      const month = String(fecha.getMonth() + 1).padStart(2, '0');
      ...
      // Estructura: social-media/whatsapp/2025/11/25/img_1234567890.jpg
      const key = `social-media/whatsapp/${year}/${month}/${day}/img_${timestamp}.jpg`;
      ...
      console.log(`✅ Imagen subida a S3 para WhatsApp: ${result.Location}`);
      return result.Location;
    } catch (error) {
      console.error('❌ Error al subir imagen a S3:', error);
      throw new Error('No se pudo subir la imagen a S3');
